...
export class Scraper {
  ...
  private getPageNumbers(document: Document) {
    ...
    const lastPageNumberElement = document.querySelector(this.paginationOptions.lastPageNumberSelector);

    if (!lastPageNumberElement) {
      throw new Error(`Failed to fetch page count, cannot scrape with pagination. Aborted.`);
    }
  ...
  public async run() {
    ...
    if (pageNumbers) {
      ...
      const scrapePromises = pageNumbers.map(async (pageNumber) => {
        ...
        const payload = await fetchPage(createPaginatedUrl(this.props, pageNumber));
